...
// Univora Feature Cards
const cardData: BentoCardProps[] = [
    { color: '#060010', title: 'Controlled Ecosystem', description: 'All platforms under one secure umbrella', label: 'Security' },
    { color: '#060010', title: 'High Performance', description: 'Lightning-fast streaming', label: 'Speed' },
    { color: '#060010', title: 'Unified Access', description: 'One account for all platforms - CinemaHub, Groovia, Skillora, Anipaca. Seamless switching without multiple logins.', label: 'Experience' },
    { color: '#060010', title: 'Mobile First', description: 'Every platform designed for all devices - phones, tablets, desktops. Responsive and beautiful everywhere.', label: 'Design' },
    { color: '#060010', title: 'Purpose-Built', description: 'Specialized platforms for each need', label: 'Focus' },
    { color: '#060010', title: 'Always Evolving', description: 'Continuous updates and new features', label: 'Growth' }
];
        ...
        <>
            ...
            <BentoCardGrid gridRef={gridRef}>
                {cardData.map((card, index) => {
                    ...
                    return enableStars ? (
                        <ParticleCard key={index} className={baseClassName} style={cardStyle} disableAnimations={shouldDisableAnimations} particleCount={particleCount} glowColor={glowColor} enableTilt={enableTilt} clickEffect={clickEffect} enableMagnetism={enableMagnetism}>{content}</ParticleCard>
                    ) : (
                        <div key={index} className={baseClassName} style={cardStyle}>{content}</div>
                    );
                })}
            </BentoCardGrid>
        </>
